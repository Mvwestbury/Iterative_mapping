...
bwa aln -l 999 -n 0.04 -t $1 $5 $2 | bwa samse $5 - $2 | samtools view -F 4 -q $7 -@ $1 -uS - | samtools sort -@ $1 -o $4/$3.sort.bam
...
angsd -dofasta 3 -minq 30 -minmapq $7 -i $4/$3.${num}.rmdup.sort.bam -out $4/$3_$6.con.${num} -docounts 1 -setMinDepth 3
gunzip $4/$3_$6.con.${num}.fa.gz
bwa index $4/$3_$6.con.${num}.fa
NUM=$(echo ${num} | awk '{print $1+1}')
bwa aln -l 999 -n 0.04 -t $1 $4/$3_$6.con.${num}.fa $2 | bwa samse $4/$3_$6.con.${num}.fa - $2 | samtools view -F 4 -q $7 -@ $1 -uS - | samtools sort -@ $1 -o $4/$3.sort.bam
samtools rmdup -S $4/$3.sort.bam $4/$3.rmdup.bam
...
samtools view -c $4/$3.${num}.rmdup.sort.bam > $4/mapped_reads.tmp
samtools view -c $4/$3.${NUM}.rmdup.sort.bam > $4/mapped_reads2.tmp
...
angsd -dofasta 3 -minq 30 -minmapq 30 -i $4/$3.${NUM}.rmdup.sort.bam -out $4/$3_$6.con.${NUM}_final -docounts 1 -setMinDepth 3
break
fi
